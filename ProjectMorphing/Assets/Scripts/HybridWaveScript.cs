...
public class HybridWaveScript : MonoBehaviour {
	
	public GameObject masterHybridSpawnManager;
	...
	public TotalCountHybrid[] EasyWavesVariants;
	public TotalCountHybrid[] MediumWavesVariants;
	public TotalCountHybrid[] HardWavesVariants;
	
	// Use this for initialization
	void Start () {
		spawnManager = (GameObject)Instantiate (masterHybridSpawnManager, new Vector3 (), Quaternion.identity);
		
		// initialize arrays
		foreach(TotalCountHybrid tc in EasyWavesVariants)
			listOfEasyWaves.Add(() => Spawn (tc.firstCount, tc.secondCount, tc.thirdCount));
		
		foreach(TotalCountHybrid tc in HardWavesVariants)
			listOfMediumWaves.Add(() => Spawn (tc.firstCount, tc.secondCount, tc.thirdCount));
		
		foreach(TotalCountHybrid tc in HardWavesVariants)
			listOfHardWaves.Add(() => Spawn (tc.firstCount, tc.secondCount, tc.thirdCount));
	...
	void Spawn(int firstObjCount, int secondObjCount = 0, int thirdObjCount = 0)
	{
		for (int i = 0; i < firstObjCount; ++i)
			spawnManager.GetComponent<HybridSpawnScript> ().SpawnEnemy (0);
		
		for(int i = 0; i < secondObjCount; ++i)
			spawnManager.GetComponent<HybridSpawnScript> ().SpawnEnemy (1);
		
		for(int i = 0; i < thirdObjCount; ++i)
			spawnManager.GetComponent<HybridSpawnScript> ().SpawnEnemy (2);
	}
